...
- **Key terms (capitalized proper nouns):** GrooveViewer, Bloody Finger Music,
  "Scan, don't ship" (the licensing architecture), SSD5, EZdrummer, Superior Drummer,
  Groove Monkee
- **Brand voice notes:** see `brand-voice` skill — Bloody Finger voice applies
- **Stack + deploy target:** leaning macOS-native SwiftUI desktop app (open decision as
  of 2026-07-11)
- **Active platform testing regimen:** Swift / macOS (native) — see *Platform testing
  regimens* above
- **Non-negotiable:** Scan, don't ship. Never bundle groove MIDI or catalog data derived
  from commercial libraries. See `.claude/rules/00-session-protocol.md`.
...
- Living "where we left off": `.claude/STATUS.md` (single status file — matches the
  sibling repos' convention; no `docs/claude/STATUS.md` duplicate).
